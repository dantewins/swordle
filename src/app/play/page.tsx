"use client"

import { Delete } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { useState, useEffect } from 'react'

export default function GamePage() {
    const wordLength = 5 // Change this for different word lengths
    const maxGuesses = 6 // Change this for different number of guesses
    const words = [
        'APPLE',
        'BERRY',
        'CHARM',
        'DANCE',
        'EAGLE',
        'FRAME',
        'GRAZE',
        'HOUSE',
        'IMAGE',
        'JUICE',
    ]

    const [secret, setSecret] = useState<string>('')
    const [history, setHistory] = useState<{ guess: string; result: Result[] }[]>([])
    const [current, setCurrent] = useState<string>('')
    const [keyStates, setKeyStates] = useState<Record<string, Result>>({})
    const [gameOver, setGameOver] = useState<boolean>(false)
    const [won, setWon] = useState<boolean>(false)

    type Result = 'correct' | 'present' | 'absent'

    const computeResult = (guess: string, secretWord: string): Result[] => {
        const res: Result[] = Array(wordLength).fill('absent') as Result[]
        const secretCount: Record<string, number> = {}

        for (const char of secretWord) {
            secretCount[char] = (secretCount[char] || 0) + 1
        }

        for (let i = 0; i < wordLength; i++) {
            if (guess[i] === secretWord[i]) {
                res[i] = 'correct'
                secretCount[guess[i]]--
            }
        }

        for (let i = 0; i < wordLength; i++) {
            if (res[i] === 'absent' && (secretCount[guess[i]] ?? 0) > 0) {
                res[i] = 'present'
                secretCount[guess[i]]--
            }
        }

        return res
    }

    const updateKeyStates = (result: Result[], guess: string) => {
        setKeyStates(prev => {
            const next = { ...prev }

            for (let i = 0; i < wordLength; i++) {
                const char = guess[i]
                const state = result[i]

                if (state === 'correct') {
                    next[char] = 'correct'
                } else if (state === 'present' && next[char] !== 'correct') {
                    next[char] = 'present'
                } else if (state === 'absent' && !next[char]) {
                    next[char] = 'absent'
                }
            }

            return next
        })
    }

    const handleKey = (key: string) => {
        if (gameOver) return

        if (key === 'BACKSPACE') {
            setCurrent(prev => prev.slice(0, -1))
        } else if (key === 'ENTER') {
            if (current.length !== wordLength) return

            const result = computeResult(current, secret)
            setHistory(prev => [...prev, { guess: current, result }])
            updateKeyStates(result, current)

            if (current === secret) {
                setWon(true)
                setGameOver(true)
            } else if (history.length + 1 === maxGuesses) {
                setGameOver(true)
            }

            setCurrent('')
        } else if (current.length < wordLength && /^[A-Z]$/.test(key)) {
            setCurrent(prev => prev + key)
        }       
    }

    const getButtonClass = (key: string): string => {
        const base = 'text-xl font-bold rounded-sm shadow-none'

        if (key === 'ENTER' || key === 'BACKSPACE') {
            return `${base} bg-gray-200 text-black hover:bg-gray-300`
        }

        const state = keyStates[key]
        if (state === 'correct') return `${base} bg-green-500 text-white hover:bg-green-600`
        if (state === 'present') return `${base} bg-yellow-500 text-white hover:bg-yellow-600`
        if (state === 'absent') return `${base} bg-gray-500 text-white hover:bg-gray-600`

        return `${base} bg-gray-200 text-black hover:bg-gray-300`
    }

    useEffect(() => {
        setSecret(words[Math.floor(Math.random() * words.length)])
    }, [])

    useEffect(() => {
        const onKeyDown = (e: KeyboardEvent) => {
            if (gameOver) return

            if (e.key === 'Enter') handleKey('ENTER')
            else if (e.key === 'Backspace') handleKey('BACKSPACE')
            else if (/^[a-z]$/i.test(e.key)) handleKey(e.key.toUpperCase())
        }

        window.addEventListener('keydown', onKeyDown)
        return () => window.removeEventListener('keydown', onKeyDown)
    }, [gameOver, current, history, secret])

    const row1 = ['Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P']
    const row2 = ['A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L']
    const row3 = ['ENTER', 'Z', 'X', 'C', 'V', 'B', 'N', 'M', 'BACKSPACE']

    return (
        <div className="flex min-h-[91vh] flex-col items-center justify-center bg-white gap-4 sm:gap-6 p-4">
            <div
                className="w-full max-w-[90vw] sm:w-96 grid gap-0.5 sm:gap-1"
                style={{
                    aspectRatio: `${wordLength}/${maxGuesses}`,
                    gridTemplateColumns: `repeat(${wordLength}, minmax(0, 1fr))`,
                    gridTemplateRows: `repeat(${maxGuesses}, minmax(0, 1fr))`,
                } as React.CSSProperties}
            >
                {Array.from({ length: wordLength * maxGuesses }).map((_, index) => {
                    const r = Math.floor(index / wordLength)
                    const c = index % wordLength

                    let letter = ''
                    let bg = 'bg-white'
                    let text = 'text-black'

                    if (r < history.length) {
                        const { guess, result } = history[r]
                        letter = guess[c]

                        if (result[c] === 'correct') {
                            bg = 'bg-green-600'
                            text = 'text-white'
                        } else if (result[c] === 'present') {
                            bg = 'bg-yellow-400'
                            text = 'text-white'
                        } else {
                            bg = 'bg-gray-500'
                            text = 'text-white'
                        }
                    } else if (r === history.length && c < current.length) {
                        letter = current[c]
                    }

                    return (
                        <div
                            key={index}
                            className={`border-2 flex items-center justify-center text-3xl font-bold ${letter ? 'border-gray-400' : 'border-gray-300'} ${bg} ${text}  ${letter ? 'animate-pop' : ''}`}
                        >
                            {letter}
                        </div>
                    );
                })}
            </div>

            <div className="w-full max-w-[90vw] flex flex-col gap-1 sm:gap-2">
                {[row1, row2, row3].map((row, rowIdx) => (
                    <div key={rowIdx} className="flex justify-center gap-0.5 sm:gap-1">
                        {row.map(key => {
                            if (key === 'ENTER') {
                                return (
                                    <Button
                                        key={key}
                                        onClick={() => handleKey(key)}
                                        className={`${getButtonClass(key)} w-16 sm:w-20 h-12`}
                                    >
                                        Enter
                                    </Button>
                                )
                            }

                            if (key === 'BACKSPACE') {
                                return (
                                    <Button
                                        key={key}
                                        onClick={() => handleKey(key)}
                                        className={`${getButtonClass(key)} w-10 sm:w-12 h-12 flex items-center justify-center`}
                                    >
                                        <Delete className="!w-6 !h-6" />
                                    </Button>
                                )
                            }

                            return (
                                <Button
                                    key={key}
                                    onClick={() => handleKey(key)}
                                    className={`${getButtonClass(key)} w-10 sm:w-12 h-12`}
                                >
                                    {key}
                                </Button>
                            )
                        })}
                    </div>
                ))}
            </div>

            {gameOver && (
                <div className="text-2xl font-bold mt-4">
                    {won ? 'You won!' : `Game over! The word was ${secret}.`}
                </div>
            )}
        </div>
    )
}
